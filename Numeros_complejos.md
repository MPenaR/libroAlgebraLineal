...
El cuerpo de los números complejos es un conjunto numérico que _en cierto modo_ contiene a los [reales](#numeros_reales), y con unas operaciones que _en cierto modo_ generalizan las conocidas en los reales. 

En los números reales podemos distinguir dos subconjuntos por como se comportan con respecto al producto. Por una parte están los números no negativos, es decir, aquellos mayores o iguales que cero, cuyo producto es también un número del mismo subconjunto (es decir, no negativo). Y por otra parte están los números negativos, que cumplen que su producto no devuelve un número negativo. 

:::{figure} x_al_cuadrado.jpg
:::

La consecuencia inmediata es que el cuadrado de un número real nunca será un número negativo y por lo tanto la ecuación $x^2 + 1 = 0$, donde "$1$", "$0$" y "$x$" representan a números reales, no admite ninguna solución. 

Esto no debería de resultar sorprendente, pues no es la primera vez que ocurre algo similar. Por ejemplo, la ecuación $n + 3 = 2$ donde "$n$", "$3$" y "$2$" representan números naturales tampoco admite ninguna solución. Sería como preguntarse *¿Cuantas manzanas debería tener un árbol, de manera que si le añado $3$ manzanas acabe teniendo $2$ manzanas?* Dicho problema no tiene solución. 

Un problema diferente sería *¿En qué piso me tengo que encontrar, si tras subir $3$ pisos acabo en el segundo piso?*. La respuesta es que tengo que estar en el sótano, es decir, en el piso $-1$. De ese modo, la ecuación $p+3=2$ donde "$p$", "$3$" y "$2$", representan números enteros, sí que tiene solución. Es decir, ecuaciones *aparentemente* iguales podrán tener solución o no dependiendo de si las cantidades hacen referencia a números naturales, enteros, etc...

En esta sección vamos a construir *otro* conjunto numérico, al que llamaremos el [cuerpo](#cuerpo) de los números complejos el cual contendrá, entre otras, cantidades que también representarán a los números reales y en el cual sí que existirá la solución a la ecuación $z^2 + 1 = 0$.
...
Podemos imaginarnos que apareciese por ejemplo la expresión 

$$
(3-\sqrt{-16})(5+\sqrt{-16})
$$

Dicha expresión no tiene sentido si asumimos que $3$ y $-16$ son símbolos que representan números reales, pues involucra la cantidad $\sqrt{-16}$ que no existe. Sin embargo, si operamos con ella como si se tratasen de números reales, es decir, aplicando las propiedades distributiva de la multiplicación con respecto a la suma y la propiedad conmutativa de la multiplicación, llegaríamos a que dicha expresión es equivalente a $3^2 - \sqrt{-16}^2$. Finalmente si, suponemos que $\sqrt{a}^2=a$, como ocurre cuando $a$ es un número real positivo, tendríamos que la expresión anterior sería equivalente a $9-(-16)=25$.

:::{important}
Esto no significa que en los reales ambas expresiones sean equivalentes, la primera no tiene sentido. Es solo una aclaración histórica de cómo se empezaron a usar conceptos que realmente hacían referencia a números complejos. Dicho de otra manera, sólo se usaron números complejos en aquellas ocasiones en los que el resultado final dejaba de incluir esas cantidades sin sentido, las raíces de números negativos.
:::

En ningún momento se trató de buscar soluciones a la ecuación real $x^2 + 1 = 0$. Era un hecho conocido que no existen números reales cuyo cuadrado sea un número negativo y no se disponía de problemas en los que fuese necesario resolver una ecuación similar. En este caso el uso es indirecto, como herramienta que permite calcular soluciones reales de ecuaciones de tercer grado.

Podemos preguntarnos, ¿Que pasaría si existiese un *número*, llamemosle $i$,  que *cumpliese* que $i^2 = -1$? Claramente dicho número tendría que ser distinto a cualquier otro número real. Es decir, podemos comprobar como funcionaría la suma y el producto de expresiones de la forma $a+bi$. Es fácil comprobar que si asumimos que la cantidad $i$ satisface las mismas propiedades que los reales para la suma y el producto, es decir, asociatividad, conmutatividad, etc, se tiene para la suma:
